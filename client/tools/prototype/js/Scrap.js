...
Scrap.prototype.edit = function (selectAll) {
  ...
  // Default block editor
    ...
    Prototype.contentEditor.focus(this.selector(), selectAll)
...
//    this.div.attr('selector', this.selector())
    this.div.tag = 'div'
    $('#PrototypeStageBody').append(this.div.toHtml())
    return this
  }
...
Scrap.prototype.selector = function () {
  ...
  if (!selector)
    return ''
  return '#PrototypeStageBody>#' + selector
}
...
Scrap.prototype.toHtml = function (context) {
  ...
  this.div.addClass('scrap')
  this.div.attr('path', this.getPath())
//  this.div.attr('page', Prototype.stage.activePage)
  this.div.attr('selector', this.selector())
...
Scrap.selectOnTap =  function (event) {
  ...
  if (Prototype.Mouse.down && Prototype.Mouse.down.altKey) {
    if (!$(this).hasClass('selection') && $('.selection').length) {
      Prototype.stage.selection.nest($(this).attr('path'))
      return false
    }
  }
