Pages.styleEditor = {}
Pages.styleEditor.livePreviewScrap = false
Pages.styleEditor.livePreviewTimeout = false
Pages.styleEditor.livePreviewStart = function () {
  clearTimeout(Pages.styleEditor.livePreviewTimeout)
  Pages.styleEditor.livePreviewTimeout = setTimeout('Pages.styleEditor.livePreview()', 500)
}
Pages.styleEditor.livePreview = function () {
  var text_area = $('#PagesStyleEditorCssEditor')
  var scrap = Pages.styleEditor.livePreviewScrap
  scrap.set('style', new Space(text_area.val()))
  scrap.element().attr('style', '').css(scrap.get('style').values)
}

Pages.styleEditor.edit = function (scrap) {
  
  $('.handle').remove()
  
  // Insert Modal
  var modal_screen = $('<div id="PagesStyleEditorModal"/>')
  ...
  // width 295px
  // height 320px
  var styleEditor = $('<div id="PagesStyleEditor"></div>')
  ...
  // Insert top bar
  var top_edit_bar = $('<div id="PagesStyleEditorTopBar"></div>')
   ...
   */
   ...
   duplicate.on("click", function(){
     Pages.stage.selection.duplicate();
   })
   ...
   lock.on('tap', function () {
     Pages.stage.selection.patch('locked true')
     save_button.trigger('click')
     return false
   })
   ...
   noLink.on('click', function () {
     Pages.stage.selection.patch('href ')
   })
   ...
   _.each(Project.values.pages.values, function (value, name) {
     ...
     link.on('click', function () {
       Pages.stage.selection.patch('tag a\nhref ' + $(this).attr('value'))
     })
   ...
   newPageLink.on('click', function () {
     var linkUrl = prompt('Enter the name of your new page', Pages.nextName(Pages.stage.activePage))
     if (linkUrl) {
       linkUrl = Permalink(linkUrl)
       Pages.stage.selection.patch('tag a\nhref ' + linkUrl)
       save_button.trigger('click')
       var currentPage = Pages.stage.activePage
       Pages.duplicatePage(null, linkUrl, true)
       Pages.stage.open(currentPage)
     }
   })
  ...
  buttonRoundness1.on('click', function () {
    Pages.stage.selection.cssPreview('border-radius 0px 0px 0px 0px')
    Pages.stage.commit()
  ...
  buttonRoundness2.on('tap', function () {
    Pages.stage.selection.cssPreview('border-radius 4px 4px 4px 4px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonRoundness3.on('tap', function () {
    Pages.stage.selection.cssPreview('border-radius 10px 10px 10px 10px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonRoundness4.on('tap', function () {
    Pages.stage.selection.cssPreview('border-radius 50px 50px 50px 50px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonBorder2.on('tap', function () {
    Pages.stage.selection.cssPreview('border none')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonBorder3.on('tap', function () {
    Pages.stage.selection.cssPreview('border 1px solid ' + currentBorderColor)
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonBorder4.on('tap', function () {
    Pages.stage.selection.cssPreview('border 5px solid ' + currentBorderColor)
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonOpacity1.on('tap', function () {
    Pages.stage.selection.cssPreview('opacity 1')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonOpacity2.on('tap', function () {
    Pages.stage.selection.cssPreview('opacity .8')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonOpacity3.on('tap', function () {
    Pages.stage.selection.cssPreview('opacity .5')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonOpacity4.on('tap', function () {
    Pages.stage.selection.cssPreview('opacity .2')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonShadow1.on('tap', function () {
    Pages.stage.selection.cssPreview('box-shadow none')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonShadow2.on('tap', function () {
    Pages.stage.selection.cssPreview('box-shadow 0px 1px 3px rgba(0,0,0,.6)')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonShadow3.on('tap', function () {
    Pages.stage.selection.cssPreview('box-shadow inset 0px 1px 3px rgba(0,0,0,.6)')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonShadow4.on('tap', function () {
    Pages.stage.selection.cssPreview('box-shadow 0px 0px 0px 4px rgba(0,0,0,.2)')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  // Insert text editor
  var textEditorContainer = $('<div id="PagesStyleEditorTextEditor"></div>')
  ...
  buttonFontFamily.on('change', function () {
    ...
    Pages.stage.selection.cssPreview('font-family ' + currentFont)
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonFontSize.on('change', function () {
    ...
    Pages.stage.selection.cssPreview('font-size ' + currentFontSize + 'px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonResize.on('click', function () {
    Pages.stage.selection.cssPreview({
    "width" : "auto",
    "height" : "auto"
    })
    Pages.stage.commit()
  ...
  buttonAlignment1.on('click', function () {
    Pages.stage.selection.cssPreview('text-align left')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonAlignment2.on('click', function () {
    Pages.stage.selection.cssPreview('text-align center')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonAlignment3.on('click', function () {
    Pages.stage.selection.cssPreview('text-align right')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonAlignment4.on('click', function () {
    Pages.stage.selection.cssPreview('text-align justify')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonPadding1.on('click', function () {
    Pages.stage.selection.cssPreview('padding 0px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonPadding2.on('click', function () {
    Pages.stage.selection.cssPreview('padding 10px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonPadding3.on('click', function () {
    Pages.stage.selection.cssPreview('padding 20px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonPadding4.on('click', function () {
    Pages.stage.selection.cssPreview('padding 40px')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonLineHeight1.on('click', function () {
    Pages.stage.selection.cssPreview('line-height 100%')
    return false
  })
  ...
  buttonLineHeight2.on('click', function () {
    Pages.stage.selection.cssPreview('line-height 140%')
    return false
  })
  ...
  buttonLineHeight3.on('click', function () {
    Pages.stage.selection.cssPreview('line-height 180%')
    return false
  })
  ...
  buttonLineHeight4.on('click', function () {
    ...
    Pages.stage.selection.cssPreview('line-height ' + currentHeight)
    return false
  })
  ...
  buttonTextStyle1.on('click', function () {
    Pages.stage.selection.cssPreview('font-weight bold')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonTextStyle2.on('click', function () {
    Pages.stage.selection.cssPreview('font-style italic')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  buttonTextStyle3.on('click', function () {
    Pages.stage.selection.cssPreview('text-decoration underline')
    Pages.stage.commit()
    $('.handle').trigger('update')
    return false
  })
  ...
  // Insert text_area
  var text_area = $('<textarea id="PagesStyleEditorCssEditor"></textarea>')
  if (scrap.get('style'))
    text_area.val(scrap.get('style'))
  text_area.on('tap mousedown click slide slidestart slideend mouseup', function (event) {
    event.stopPropagation()
  })
  Pages.styleEditor.livePreviewScrap = scrap
  text_area.on('keyup', Pages.styleEditor.livePreviewStart)
  
  styleEditor.append(text_area)
  
  // Insert bottom bar
  var button_container = $('<div id="PagesStyleEditorBottomBar"></div>')
  text_area.append(button_container)
  
  // Insert save button
  var save_button = $('<div id="PagesStyleEditorSaveButton">Save</div>')
  ...
  $(".colorPicker").spectrum({
      color: currentColor,
      move: function(color) {
        Pages.stage.selection.cssPreview('background-color ' + color.toHexString());
  ...
  $(".colorBorderPicker").spectrum({
      ...
      move: function(color) {
        Pages.stage.selection.cssPreview('border-color ' + color.toHexString());
  ...
  $(".colorFontPicker").spectrum({
      ...
      move: function(color) {
        Pages.stage.selection.cssPreview('color ' + color.toHexString());
        currentBorderColor = color.toHexString();
      },
      change: function(color) {
          $("#basic-log").text("change called: " + color.toHexString());
  ...
  $('.sp-input[type=text]').on('keydown', function(e) {
      if (e.which == 13) {
          ...
          Pages.stage.selection.css(colorType + inputColor);
          return false
      }
  });
 ...
 // scroll to reveal the styleEditor
  var difference = styleEditor.position().top + styleEditor.outerHeight() - Pages.stage.height()
  if (difference > 0) {
    $('#PagesStage').scrollTop($('#PagesStage').scrollTop() + difference)
  ...
  if (difference > 0) {
    $('#PagesStage').scrollLeft($('#PagesStage').scrollLeft() + difference)
