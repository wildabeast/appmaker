 ...
 * @param {string}
 */
Pages.StretchHandle.create = function (scrap, row, column, fixed) {
  ...
  div.on("mousedown", Pages.StretchHandle.mousedown)
  div.on("slide", Pages.StretchHandle.slide)
  div.on("slidestart", Pages.StretchHandle.slidestart)
  div.on("slideend", Pages.StretchHandle.slideend)
  div.on("tap", Pages.StretchHandle.tap)
  div.on("update", Pages.StretchHandle.update)
  div.on("dblclick", Pages.StretchHandle.dblclick)
  div.trigger("update")
}
 ...
 */
Pages.StretchHandle.dimensions = {}

/**
 * If small scrap is on top of (higher z-index) a bigger scrap, selects small scrap
 */
Pages.StretchHandle.mousedown = function () {
  Pages.StretchHandle.dimensions = $(this).owner().dimensions()
  var scrap = $(this).owner().scrap()
  Pages.StretchHandle.originalWidth = parseFloat(scrap.get('style width'))
  Pages.StretchHandle.originalHeight = parseFloat(scrap.get('style height'))
  Pages.grid.create()
}
 ...
 */
Pages.StretchHandle.slide = function () {
  ...
  if (column === 'left')
    x0 = Pages.StretchHandle.dimensions.right
  else if (column === 'right')
    x0 = Pages.StretchHandle.dimensions.left
  else
    x0 = Pages.StretchHandle.dimensions.center
  
  if (row === 'top')
    y0 = Pages.StretchHandle.dimensions.bottom
  else if (row === 'bottom')
    y0 = Pages.StretchHandle.dimensions.top
  else
    y0 = Pages.StretchHandle.dimensions.middle
  
  var x1 = Pages.Mouse.move.pageX - $(this).parent().offset().left // + scroll left
  var y1 = Pages.Mouse.move.pageY - $(this).parent().offset().top// Pages.stage.scrollTop()// + scroll top
//  console.log(x1)
  
  // todo: fix bug where offset changes
  
  var length = Pages.StretchHandle.getLength(x0, y0, x1, y1,
  ...
  if (fixed) {
    var change = parseFloat(scrap.get('style width')) / Pages.StretchHandle.originalWidth
    scrap.set('style height', Math.round(Pages.StretchHandle.originalHeight * change) + 'px')
  }
 ...
 */
Pages.StretchHandle.slidestart = function (event) {
  var owner = $(this).owner()
  var scrap = owner.scrap()
...
Pages.StretchHandle.dblclick = function () {
 ...
 */
Pages.StretchHandle.slideend = function () {
 ...
 */
Pages.StretchHandle.getLength = function (x0, y0, x1, y1, x_snap, y_snap) {
  var length = { x : x1 - x0, y: y1 - y0}
  
  var grid_change = Pages.grid.getDelta([
     {x : x1, y : y1}
  ])
 ...
 */
Pages.StretchHandle.tap = function () {
  return false
}
 ...
 */
Pages.StretchHandle.update = function () {
  var element = $(this).owner()
  var left,
      _top,
      row = $(this).data().row,
