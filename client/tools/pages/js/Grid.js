...
Grid.prototype.addDynamicPoints = function () {
  ...
  if ($('#PagesStageBody #body').children('.scrap:not(.selection)').length > 200)
    return true
  $('#PagesStageBody #body').children('.scrap:not(.selection)').each(function(){
...
Grid.prototype.addFixedPoints = function () {
  ...
  // verticals
  var start = $('#PagesStageBody').position().left
  var end = $('#PagesStageBody').position().left + $('#PagesStageBody').outerWidth()
  this.horizontal_spacing = parseFloat(this.horizontal_spacing)
  for (var i = start; i <= end; i = i + this.horizontal_spacing) {
    this.addPoint(i, 0, '#PagesStageBody')  
  }
  
  start = $('#PagesStageBody').position().top
  end = $('#PagesStageBody').position().top + $('#PagesStageBody').outerHeight()
  this.vertical_spacing = parseFloat(this.vertical_spacing)
  for (var i = start; i <= end; i = i + this.vertical_spacing) {
    this.addPoint(0, i, '#PagesStageBody')  
  }
...
//    this.points.clear()
  ...
  if (this.snap_to_container) {
    ...
    // left
//    this.addPoint(0, $('#PagesStageBody').position().top, '#PagesStageBody')
    // right
    this.addPoint($('#PagesStageBody').outerWidth(), 0, '#PagesStageBody')
    // center
    this.addPoint(Math.round($('#PagesStageBody').outerWidth()/2), 0, '#PagesStageBody')
    // top
    this.addPoint(0, 0, '#PagesStageBody')
    // bottom
    this.addPoint(0, $('#PagesStageBody').height(), '#PagesStageBody')
    // middle
    this.addPoint(0, Math.round($(window).height()/2), '#PagesStageBody')
...
Grid.prototype.draw = function () {
  ...
  for (var i in this.points) {
    ...
    if (point.selector !== '#PagesStageBody')
      continue
...
//  console.log(point2.selector)
  ...
  // Dont draw lines based on objects
  if (point2.selector !== '#PagesStageBody')
...
Grid.prototype.drawSnaplineCanvas = function () {
  ...
  this.width = $("#PagesStageBody").width()
  this.height = $("#PagesStageBody").height()
