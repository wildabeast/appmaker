...
Element.prototype.toHtml=function(){var a="<"+this.tag,b;for(b in this.attrs)this.attrs.hasOwnProperty(b)&&(a+=" "+b+'="'+this.attrs[b]+'"');"meta"===this.tag?a+=' content="'+this.content+'">':(a+=">"+this.content,a+="</"+this.tag+">");return a};
function Scrap(a,b){this.path=a;this.id=a.split(/ /g).pop();this.clear();b instanceof Space||(b=new Space(b));this.patch(b);var c=this.values.scraps;if(c instanceof Space)for(var d in c.keys){var e=c.keys[d];c.values[e]=new Scrap(a+" "+e,c.values[e])}return this}Scrap.format=function(a,b){return"nl2br"===b?a.toString().replace(/\n/g,"<br>"):"markdown"===b?marked(a):a};
Scrap.getPointer=function(a,b){if(!a||"string"===typeof b||!b&&"undefined"===typeof window)return null;b||(b=window);var c=a.split("."),d=c.shift();return!(d in b)?null:!c.length?b[d]:Scrap.getPointer(c.join("."),b[d])};Scrap.replace=function(a,b){return a.toString().replace(/\{\{([_a-zA-Z0-9\.]+)( [^\}]+)?\}\}/g,function(a,d,e){return Scrap.getPointer(d,b)||(e?e.substr(1):"")})};
...
Scrap.prototype.setContent=function(a,b){b||(b={});if(this.values.loop)return this.loop(a);this.values.content&&this.div.html(Scrap.format(Scrap.replace(this.values.content,a),this.values.content_format));if(this.values.styles&&this.values.styles instanceof Space){var c=this.div;this.values.styles.each(function(b,d){c.html(Scrap.styleToCss(b,d.values,a))})}if(this.values.scraps)for(var d in this.values.scraps.keys){var e=this.values.scraps.keys[d];(b.draft||"true"!==this.values.scraps.values[e].values.draft)&&
this.div.html(this.values.scraps.values[e].toHtml(a))}return this};Scrap.prototype.setElementTag=function(a){tag=this.values.tag?this.values.tag:"div";this.div=new Element(tag);this.div.attr("id",this.id);this.each(function(b){if(b.match(/^(scraps|content|tag|loop|on.*|style|draft)$/))return!0;this.setProperty(b,a)})};Scrap.prototype.setProperty=function(a,b){this.values[a]&&this.div.attr(a,Scrap.replace(this.values[a],b))};
