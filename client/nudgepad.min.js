...
g!=g.window?delete g[q]:g[q]=null)}}}function n(j,a,y){if(y===b&&1===j.nodeType){var u="data-"+a.replace(wa,"-$1").toLowerCase();if(y=j.getAttribute(u),"string"==typeof y){try{y="true"===y?!0:"false"===y?!1:"null"===y?null:+y+""===y?+y:Ga.test(y)?e.parseJSON(y):y}catch(c){}e.data(j,a,y)}else y=b}return y}function t(j){for(var a in j)if(("data"!==a||!e.isEmptyObject(j[a]))&&"toJSON"!==a)return!1;return!0}function r(){return!0}function m(){return!1}function D(j,a){do j=j[a];while(j&&1!==j.nodeType);
...
h.attributes;c.length>d;d++)u=c[d].name,u.indexOf("data-")||(u=e.camelCase(u.slice(5)),n(h,u,g[u]));e._data(h,"parsedAttrs",!0)}return g}return"object"==typeof j?this.each(function(){e.data(this,j)}):e.access(this,function(a){return a===b?h?n(h,j,e.data(h,j)):null:(this.each(function(){e.data(this,j,a)}),b)},null,a,1<arguments.length,null,!0)},removeData:function(j){return this.each(function(){e.removeData(this,j)})}});e.extend({queue:function(j,a,c){var u;return j?(a=(a||"fx")+"queue",u=e._data(j,
...
v,s))),g.add&&(g.add.call(a,z),z.handler.guid||(z.handler.guid=h.guid)),d?k.splice(k.delegateCount++,0,z):k.push(z),e.event.global[v]=!0;a=null}},remove:function(a,b,c,h,d){var g,f,q,p,s,z,k,v,x,M,l,m=e.hasData(a)&&e._data(a);if(m&&(z=m.events)){b=(b||"").match(Ba)||[""];for(s=b.length;s--;)if(q=Jb.exec(b[s])||[],x=l=q[1],M=(q[2]||"").split(".").sort(),x){k=e.event.special[x]||{};x=(h?k.delegateType:k.bindType)||x;v=z[x]||[];q=q[2]&&RegExp("(^|\\.)"+M.join("\\.(?:.*\\.|)")+"(\\.|$)");for(p=g=v.length;g--;)f=
v[g],!d&&l!==f.origType||c&&c.guid!==f.guid||q&&!q.test(f.namespace)||h&&h!==f.selector&&("**"!==h||!f.selector)||(v.splice(g,1),f.selector&&v.delegateCount--,k.remove&&k.remove.call(a,f));p&&!v.length&&(k.teardown&&!1!==k.teardown.call(a,M,m.handle)||e.removeEvent(a,x,m.handle),delete z[x])}else for(x in z)e.event.remove(a,x+b[s],c,h,!0);e.isEmptyObject(z)&&(delete m.handle,e._removeData(a,"events"))}},trigger:function(j,c,h,u){var d,g,f,q,p,s,z=[h||F],k=C.call(j,"type")?j.type:j;s=C.call(j,"namespace")?
...
s,k=[],z=[],v=g.length,x;if(!(x=d)){x=b||"*";for(var M=f.nodeType?[f]:f,rc=[],l=0,m=M.length;m>l;l++)aa(x,M[l],rc);x=rc}x=!a||!d&&b?x:Lb(x,k,a,f,q);M=c?h||(d?a:v||e)?[]:g:x;if(c&&c(x,M,f,q),e){p=Lb(M,z);e(p,[],f,q);for(f=p.length;f--;)(s=p[f])&&(M[z[f]]=!(x[z[f]]=s))}if(d){if(h||a){if(h){p=[];for(f=M.length;f--;)(s=M[f])&&p.push(x[f]=s);h(null,M=[],p,q)}for(f=M.length;f--;)(s=M[f])&&-1<(p=h?fc.call(d,s):k[f])&&(d[p]=!(g[p]=s))}}else M=Lb(M===g?M.splice(v,M.length):M),h?h(null,g,M,q):ub.apply(g,M)})},
...
ra)),204===a)g=!0,n="nocontent";else if(304===a)g=!0,n="notmodified";else{var Q;a:{d=v;g=B;var fa,G;u={};ra=0;n=d.dataTypes.slice();Z=n[0];if(d.dataFilter&&(g=d.dataFilter(g,d.dataType)),n[1])for(fa in d.converters)u[fa.toLowerCase()]=d.converters[fa];for(;z=n[++ra];)if("*"!==z){if("*"!==Z&&Z!==z){if(fa=u[Z+" "+z]||u["* "+z],!fa)for(Q in u)if(G=Q.split(" "),G[1]===z&&(fa=u[Z+" "+G[0]]||u["* "+G[0]])){!0===fa?fa=u[Q]:!0!==u[Q]&&(z=G[0],n.splice(ra--,0,z));break}if(!0!==fa)if(fa&&d["throws"])g=fa(g);
else try{g=fa(g)}catch(Ya){Q={state:"parsererror",error:fa?Ya:"No conversion from "+Z+" to "+z};break a}}Z=z}Q={state:"success",data:g}}g=Q;n=g.state;z=g.data;u=g.error;g=!u}else u=n,(a||!n)&&(n="error",0>a&&(a=0));C.status=a;C.statusText=(c||n)+"";g?l.resolveWith(x,[z,n,C]):l.rejectWith(x,[C,n,u]);C.statusCode(r);r=b;s&&M.trigger(g?"ajaxSuccess":"ajaxError",[C,v,g?z:u]);m.fireWith(x,[C,n]);s&&(M.trigger("ajaxComplete",[C,v]),--e.active||e.event.trigger("ajaxStop"))}}"object"==typeof a&&(c=a,a=b);
c=c||{};var d,g,f,q,p,s,k,z,v=e.ajaxSetup({},c),x=v.context||v,M=v.context&&(x.nodeType||x.jquery)?e(x):e.event,l=e.Deferred(),m=e.Callbacks("once memory"),r=v.statusCode||{},ra={},n={},w=0,Z="canceled",C={readyState:0,getResponseHeader:function(a){var b;if(2===w){if(!z)for(z={};b=Ad.exec(q);)z[b[1].toLowerCase()]=b[2];b=z[a.toLowerCase()]}return null==b?null:b},getAllResponseHeaders:function(){return 2===w?q:null},setRequestHeader:function(a,b){var c=a.toLowerCase();return w||(a=n[c]=n[c]||a,ra[a]=
b),this},overrideMimeType:function(a){return w||(v.mimeType=a),this},statusCode:function(a){var b;if(a)if(2>w)for(b in a)r[b]=[r[b],a[b]];else C.always(a[C.status]);return this},abort:function(a){a=a||Z;return k&&k.abort(a),h(0,a),this}};if(l.promise(C).complete=m.add,C.success=C.done,C.error=C.fail,v.url=((a||v.url||cb)+"").replace(zd,"").replace(Cd,mb[1]+"//"),v.type=c.method||c.type||v.method||v.type,v.dataTypes=e.trim(v.dataType||"*").toLowerCase().match(Ba)||[""],null==v.crossDomain&&(d=Nc.exec(v.url.toLowerCase()),
...
Ea.css("background","-ms-"+e),Ea.css("background",e))}if(H.showInput){if(1>O&&("hex"===a||"name"===a))a="rgb";Na.val(b.toString(a))}H.showPalette&&l();n()},Y=function(){var a=Ma*da,b=ia-qa*ia,a=Math.max(-S,Math.min(da-S,a-S)),b=Math.max(-S,Math.min(ia-S,b-S));ra.css({top:b,left:a});rb.css({left:O*oa-ua/2});pb.css({top:la*na-Ha})},J=function(a){var b=W();kb&&va.val(b.toString(Ua)).change();var c=!tinycolor.equals(b,Va);Va=b;x(b);a&&c&&I.change(b)},L=function(){da=z.width();ia=z.height();S=ra.height();
...
a.keyCode&&v()});Jb.text(H.cancelText);Jb.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();F("cancel")});sb.text(H.chooseText);sb.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();Na.hasClass("sp-validation-error")||(J(!0),F())});m(qb,function(a,b,c){O=a/oa;c.shiftKey&&(O=Math.round(10*O)/10);Q()});m(Ta,function(a,b){la=parseFloat(b/na);Q()},w,E);m(z,function(a,b){Ma=parseFloat(a/da);qa=parseFloat((ia-b)/ia);Q()},w,E);yb?(e(yb),G(),Ua=La||tinycolor(yb).format,
...
a.hasOwnProperty("_tc_id"))return a;c=a;var n=0,r=0,t=0;a=1;var G=!1,D=!1;if("string"==typeof c)a:{c=c.replace(E,"").replace(Y,"").toLowerCase();var A=!1;if(ea[c])c=ea[c],A=!0;else if("transparent"==c){c={r:0,g:0,b:0,a:0};break a}c=(b=x.rgb.exec(c))?{r:b[1],g:b[2],b:b[3]}:(b=x.rgba.exec(c))?{r:b[1],g:b[2],b:b[3],a:b[4]}:(b=x.hsl.exec(c))?{h:b[1],s:b[2],l:b[3]}:(b=x.hsla.exec(c))?{h:b[1],s:b[2],l:b[3],a:b[4]}:(b=x.hsv.exec(c))?{h:b[1],s:b[2],v:b[3]}:(b=x.hex6.exec(c))?{r:parseInt(b[1],16),g:parseInt(b[2],
16),b:parseInt(b[3],16),format:A?"name":"hex"}:(b=x.hex3.exec(c))?{r:parseInt(b[1]+""+b[1],16),g:parseInt(b[2]+""+b[2],16),b:parseInt(b[3]+""+b[3],16),format:A?"name":"hex"}:!1}"object"==typeof c&&(c.hasOwnProperty("r")&&c.hasOwnProperty("g")&&c.hasOwnProperty("b")?(n=255*L(c.r,255),r=255*L(c.g,255),t=255*L(c.b,255),G=!0,D="rgb"):c.hasOwnProperty("h")&&c.hasOwnProperty("s")&&c.hasOwnProperty("v")?(D=c.h,r=c.s,t=c.v,D=6*L(D,360),r=L(r,100),t=L(t,100),n=W.floor(D),G=D-n,D=t*(1-r),b=t*(1-G*r),G=t*(1-
(1-G)*r),A=n%6,n=255*[t,b,D,D,G,t][A],r=255*[G,t,t,b,D,D][A],t=255*[D,D,G,t,t,b][A],G=!0,D="hsv"):c.hasOwnProperty("h")&&(c.hasOwnProperty("s")&&c.hasOwnProperty("l"))&&(n=c.h,D=c.s,t=c.l,r=function(a,b,c){0>c&&(c+=1);1<c&&(c-=1);return c<1/6?a+6*(b-a)*c:0.5>c?b:c<2/3?a+6*(b-a)*(2/3-c):a},n=L(n,360),D=L(D,100),t=L(t,100),0===D?D=t=b=t:(b=0.5>t?t*(1+D):t+D-t*D,G=2*t-b,D=r(G,b,n+1/3),t=r(G,b,n),b=r(G,b,n-1/3)),n=255*D,r=255*t,t=255*b,G=!0,D="hsl"),c.hasOwnProperty("a")&&(a=c.a));n=l(255,ca(n,0));r=
l(255,ca(r,0));t=l(255,ca(t,0));1>n&&(n=Q(n));1>r&&(r=Q(r));1>t&&(t=Q(t));b=G;c=c&&c.format||D;d=n;f=r;m=t;var v=ja(a);return{ok:b,format:c,_tc_id:ga++,alpha:v,toHsv:function(){var a=K(d,f,m);return{h:a.h,s:a.s,v:a.v,a:v}},toHsvString:function(){var a=K(d,f,m),b=Q(360*a.h),c=Q(100*a.s),a=Q(100*a.v);return 1==v?"hsv("+b+", "+c+"%, "+a+"%)":"hsva("+b+", "+c+"%, "+a+"%, "+v+")"},toHsl:function(){var a=X(d,f,m);return{h:a.h,s:a.s,l:a.l,a:v}},toHslString:function(){var a=X(d,f,m),b=Q(360*a.h),c=Q(100*
a.s),a=Q(100*a.l);return 1==v?"hsl("+b+", "+c+"%, "+a+"%)":"hsla("+b+", "+c+"%, "+a+"%, "+v+")"},toHex:function(){return R(d,f,m)},toHexString:function(a){return"#"+R(d,f,m,a)},toRgb:function(){return{r:Q(d),g:Q(f),b:Q(m),a:v}},toRgbString:function(){return 1==v?"rgb("+Q(d)+", "+Q(f)+", "+Q(m)+")":"rgba("+Q(d)+", "+Q(f)+", "+Q(m)+", "+v+")"},toName:function(){return g[R(d,f,m)]||!1},toFilter:function(a,b){var c=R(d,f,m,!0),e=c,g=Math.round(255*ja(v)).toString(16),p=g,s=a&&a.gradientType?"GradientType = 1, ":
...
b))return!1;return!0};k.isElement=function(a){return!!(a&&1==a.nodeType)};k.isArray=n||function(a){return"[object Array]"==m.call(a)};k.isObject=function(a){return a===Object(a)};k.isArguments=function(a){return"[object Arguments]"==m.call(a)};k.isArguments(arguments)||(k.isArguments=function(a){return!(!a||!k.has(a,"callee"))});k.isFunction=function(a){return"[object Function]"==m.call(a)};k.isString=function(a){return"[object String]"==m.call(a)};k.isNumber=function(a){return"[object Number]"==
...
m(A.paragraph)("(?!","(?!"+A.gfm.fences.source.replace(/(^|[^\[])\^/g,"$2")+"|")();A.lexer=function(a){var b=[];b.links={};a=a.replace(/\r\n|\r/g,"\n").replace(/\t/g,"    ");return A.token(a,b,!0)};A.token=function(a,b,c){a=a.replace(/^ +$/gm,"");for(var d,f,l,n,m,r;a;){if(l=A.newline.exec(a))a=a.substring(l[0].length),1<l[0].length&&b.push({type:"space"});if(l=A.code.exec(a))a=a.substring(l[0].length),l=l[0].replace(/^ {4}/gm,""),b.push({type:"code",text:!R.pedantic?l.replace(/\n+$/,""):l});else if(l=
...
b,c),b.push({type:"blockquote_end"});else if(l=A.list.exec(a)){a=a.substring(l[0].length);b.push({type:"list_start",ordered:isFinite(l[2])});l=l[0].match(A.item);d=!1;r=l.length;for(m=0;m<r;m++)n=l[m],f=n.length,n=n.replace(/^ *([*+-]|\d+\.) +/,""),~n.indexOf("\n ")&&(f-=n.length,n=!R.pedantic?n.replace(RegExp("^ {1,"+f+"}","gm"),""):n.replace(/^ {1,4}/gm,"")),f=d||/\n\n(?!\s*$)/.test(n),m!==r-1&&(d="\n"===n[n.length-1],f||(f=d)),b.push({type:f?"loose_item_start":"list_item_start"}),A.token(n,b),
...
function natural_sort(a,b){var c=/(^-?[0-9]+(\.?[0-9]*)[df]?e?[0-9]?$|^0x[0-9a-f]+$|[0-9]+)/gi,d=/(^[ ]*|[ ]*$)/g,f=/(^([\w ]+,?[\w ]+)?[\w ]+,?[\w ]+\d+:\d+(:\d+)?[\w ]?|^\d{1,4}[\/\-]\d{1,4}[\/\-]\d{1,4}|^\w+, \w+ \d+, \d{4})/,n=/^0x[0-9a-f]+$/i,t=/^0/,r=(natural_sort.insensitive&&(""+a).toLowerCase()||""+a).replace(d,"")||"",m=(natural_sort.insensitive&&(""+b).toLowerCase()||""+b).replace(d,"")||"",d=r.replace(c,"\x00$1\x00").replace(/\0$/,"").replace(/^\0/,"").split("\x00"),c=m.replace(c,"\x00$1\x00").replace(/\0$/,
"").replace(/^\0/,"").split("\x00"),r=parseInt(r.match(n))||1!=d.length&&r.match(f)&&Date.parse(r);if(f=parseInt(m.match(n))||r&&m.match(f)&&Date.parse(m)||null){if(r<f)return-1;if(r>f)return 1}r=0;for(m=Math.max(d.length,c.length);r<m;r++){f=!(d[r]||"").match(t)&&parseFloat(d[r])||d[r]||0;n=!(c[r]||"").match(t)&&parseFloat(c[r])||c[r]||0;if(isNaN(f)!==isNaN(n))return isNaN(f)?1:-1;typeof f!==typeof n&&(f+="",n+="");if(f<n)return-1;if(f>n)return 1}return 0}
...
case "boolean":case "null":return String(p);case "object":if(!p)return"null";v+=m;l=[];if("[object Array]"===Object.prototype.toString.apply(p)){g=p.length;for(h=0;h<g;h+=1)l[h]=e(h,p)||"null";q=0===l.length?"[]":v?"[\n"+v+l.join(",\n"+v)+"\n"+k+"]":"["+l.join(",")+"]";v=k;return q}if(n&&"object"===typeof n){g=n.length;for(h=0;h<g;h+=1)"string"===typeof n[h]&&(f=n[h],(q=e(f,p))&&l.push(d(f)+(v?": ":":")+q))}else for(f in p)Object.prototype.hasOwnProperty.call(p,f)&&(q=e(f,p))&&l.push(d(f)+(v?": ":
":")+q);q=0===l.length?"{}":v?"{\n"+v+l.join(",\n"+v)+"\n"+k+"}":"{"+l.join(",")+"}";v=k;return q}}if(b&&b.parse)return a.JSON={parse:b.parse,stringify:b.stringify};var f=a.JSON={},g=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,k=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,v,m,l={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},n;
...
case "Z":case "ZZ":return s;case "T":return M;case "MM":case "DD":case "YY":case "HH":case "hh":case "mm":case "ss":case "M":case "D":case "d":case "H":case "h":case "m":case "s":return J;default:return RegExp(a.replace("\\",""))}}function w(a,b){var c=[0,0,1,0,0,0,0],d={tzh:0,tzm:0},e=b.match(P),f,g;for(f=0;f<e.length;f++)if((g=(H(e[f]).exec(a)||[])[0])&&(a=a.slice(a.indexOf(g)+g.length)),ka[e[f]]){var k=c,m=d,l=void 0,n=void 0;switch(e[f]){case "M":case "MM":k[1]=null==g?0:~~g-1;break;case "MMM":case "MMMM":for(l=
0;12>l;l++)if(G().monthsParse[l].test(g)){k[1]=l;n=!0;break}n||(k[8]=!1);break;case "D":case "DD":case "DDD":case "DDDD":null!=g&&(k[2]=~~g);break;case "YY":k[0]=~~g+(70<~~g?1900:2E3);break;case "YYYY":k[0]=~~Math.abs(g);break;case "a":case "A":m.isPm="pm"===(g+"").toLowerCase();break;case "H":case "HH":case "h":case "hh":k[3]=~~g;break;case "m":case "mm":k[4]=~~g;break;case "s":case "ss":k[5]=~~g;break;case "S":case "SS":case "SSS":k[6]=~~(1E3*("0."+g));break;case "Z":case "ZZ":m.isUTC=!0,(l=(g+
...
c){if(null===b||""===b)return null;var d;if(k.isMoment(b))d=new f(new Date(+b._d),b._isUTC,b._lang);else{if(c)if(D(c)){var g=b.match(ea)||[],e,m=99,l,n;for(l=0;l<c.length;l++)n=w(b,c[l]),e=V(new f(n),c[l]).match(ea)||[],e=U(g,e),e<m&&(m=e,d=n)}else d=w(b,c);else if(d=ca.exec(b),b===a)d=new Date;else if(d)d=new Date(+d[1]);else if(b instanceof Date)d=b;else if(D(b))d=T(b);else if("string"==typeof b)if(d="YYYY-MM-DDT",F.exec(b)){for(g=0;4>g;g++)if(fa[g][1].exec(b)){d+=fa[g][0];break}d=s.exec(b)?w(b,
...
k.defaultFormat="YYYY-MM-DDTHH:mm:ssZ";k.lang=function(a,b){var c;if(!a)return W;(b||!ga[a])&&A(a,b);if(ga[a]){for(c=0;c<l.length;c++)k[l[c]]=ga[a][l[c]];k.monthsParse=ga[a].monthsParse;W=a}};k.langData=G;k.isMoment=function(a){return a instanceof f};k.isDuration=function(a){return a instanceof n};k.lang("en",{months:"January February March April May June July August September October November December".split(" "),monthsShort:"Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec".split(" "),weekdays:"Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),
...
338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,506948616,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298],n=[1779033703,3144134277,1013904242,2773480762,1359893119,2600822924,528734635,1541459225],r=Array(64),t,V,H,w,X,K,R,L,k,E,Y,ga;a[b>>5]|=128<<24-b%
32;a[(b+64>>9<<4)+15]=b;for(k=0;k<a.length;k+=16){t=n[0];V=n[1];H=n[2];w=n[3];X=n[4];K=n[5];R=n[6];L=n[7];for(E=0;64>E;E++)r[E]=16>E?a[E+k]:c(c(c(d(r[E-2],17)^d(r[E-2],19)^r[E-2]>>>10,r[E-7]),d(r[E-15],7)^d(r[E-15],18)^r[E-15]>>>3),r[E-16]),Y=c(c(c(c(L,d(X,6)^d(X,11)^d(X,25)),X&K^~X&R),f[E]),r[E]),ga=c(d(t,2)^d(t,13)^d(t,22),t&V^t&H^V&H),L=R,R=K,K=X,X=c(w,Y),w=H,H=V,V=t,t=c(Y,ga);n[0]=c(t,n[0]);n[1]=c(V,n[1]);n[2]=c(H,n[2]);n[3]=c(w,n[3]);n[4]=c(X,n[4]);n[5]=c(K,n[5]);n[6]=c(R,n[6]);n[7]=c(L,n[7])}return n},
n=function(a){for(var c=[],d=(1<<b)-1,f=0;f<a.length*b;f+=b)c[f>>5]|=(a.charCodeAt(f/b)&d)<<24-f%32;return c},t=function(a){for(var b="",c=0;c<4*a.length;c++)b+="0123456789abcdef".charAt(a[c>>2]>>8*(3-c%4)+4&15)+"0123456789abcdef".charAt(a[c>>2]>>8*(3-c%4)&15);return b},r=function(b){return b="object"==typeof b?a(b).val():b.toString()};a.extend({sha256:function(a){a=r(a);return t(f(n(a),a.length*b))},sha256hmac:function(a,c){a=r(a);c=r(c);var d,T=a;d=c;var A=n(T);16<A.length&&(A=f(A,T.length*b));
for(var G=Array(16),T=Array(16),V=0;16>V;V++)G[V]=A[V]^909522486,T[V]=A[V]^1549556828;d=f(G.concat(n(d)),512+d.length*b);d=f(T.concat(d),768);return t(d)},sha256config:function(a){b=parseInt(a)||8}});a.fn.sha256=function(b){a.sha256config(b);b=r(a(this).val());b=a.sha256(b);a.sha256config(8);return b}})(jQuery);
...
Scrap.prototype.loop=function(a){if(!this.values.loop||!this.values.scraps)return null;var b=this.values.scraps.toString(),c=this.values.loop;"string"===typeof c&&c.match(/\{\{/)&&(c=Scrap.getPointer(c.replace(/\{|\}/g,""),a));"string"===typeof c&&(c=this.values.loop.split(/ /g));if(!c)return null;c instanceof Space||(c=new Space(c));var d="";c.each(function(c,n){var t={key:c,value:n},r=b.replace(/\{\{([_a-z0-9\.]+)\}\}/gi,function(b,c){var d;return(d=Scrap.getPointer(c,t))?d:Scrap.getPointer(c,a)});
...
!1;null!=a&&(a=Object(a),a.__proto__=[a.__proto__,a.__proto__=null,c=b in a][0]);return c});return n.apply(this,arguments)}function t(a){return String(a).replace(/([ -])(?!$)/g,"$1?")}function r(a,b){var c=null,f=function(d,f){c=b(c,d,f,a)},k=-1,m=a.length;if(m==m>>>0)for(;++k<m;)f(a[k],k,a);else d(a,f);return c}function m(a){return String(a).replace(/^ +| +$/g,"")}function D(b){function n(a){return r(a,function(a,d){var f=d.pattern||t(d);if(!a&&(a=RegExp("\\b"+f+" *\\d+[.\\w_]*","i").exec(b)||RegExp("\\b"+
...
function(a){a.stopPropagation()});var n=$('<div id="nudgepadEditorSaveButton">Send</div>'),t=$('<div id="nudgepadEditorCancelButton">Close</div>'),r=$('<div id="nudgepadEditorButtonContainer"></div>');f.on("click",function(){t.trigger("click")});t.on("click",function(){n.remove();a.remove();f.remove();r.remove();t.remove();d.remove();c.remove();b.remove()});n.on("click",function(){var d=b.val(),f="nudgepad.console";c.is(":checked")&&(f="nudgepad.exec");$.post(f,{command:d},function(c){a.append(">"+
d.replace(/\n/g,"> \n")+"\n");a.append(c+"\n");a.scrollTop($("pre")[0].scrollHeight+"");b.val("");b.focus()},null,function(c){mixpanel.track("I used the console and got an error");a.append(">"+d.replace(/\n/g,"> \n")+"\n");a.append("ERROR\n");a.append(c.responseText+"\n");a.scrollTop($("pre")[0].scrollHeight+"");b.val("");b.focus()})});b.on("enterkey",function(){n.click()});$("body").append(f);$("body").append(c);$("body").append(d);$("body").append(a);$("body").append(b);$("body").append(n);$("body").append(t);
$("body").append(r);b.focus()};nudgepad.contentEditor={};nudgepad.contentEditor.onblur=function(){$(this).scrap().set("content",$(this).html());$(this).off("tap mousedown slide slidestart hold slideend",nudgepad.contentEditor.killEvent);$(this).removeAttr("contenteditable");nudgepad.stage.commit();nudgepad.broadcastSelection()};
nudgepad.contentEditor.focus=function(a,b){nudgepad.stage.selection.clear();var c=$(a),d=c.scrap();if(d.isContentEditable())nudgepad.broadcastSelection(d.selector()),c.attr("contenteditable","true"),c.on("tap slide slidestart hold slideend",nudgepad.contentEditor.killEvent),c.on("blur",nudgepad.contentEditor.onblur),c.focus(),MoveCursorToEnd(c[0]),b&&document.execCommand("selectAll",!1,null);else{var f="content",n=d.values.tag;if(n&&n.match(/^(list)$/))return nudgepad.stage.selection.clear(),c.selectMe(),
nudgepad.stage.selection.editSource(),!1;n&&n.match(/^(textarea|input|password)$/)&&(f="placeholder");nudgepad.textPrompt("Editing content for this block",d.values[f],function(a){d.values[f]=a;nudgepad.stage.commit();c.remove();$("#nudgepadStageBody").append(d.toHtml(null,!0));c.selectMe()})}};nudgepad.contentEditor.killEvent=function(){nudgepad.mouse.down.stopPropagation();return!1};nudgepad.apps.develop=new App("develop");nudgepad.apps.develop.files="";nudgepad.apps.develop.log="";
nudgepad.apps.develop.path="";nudgepad.apps.develop.pathPretty="";nudgepad.apps.develop.status="";
nudgepad.apps.develop.clone=function(){var a=prompt("Enter a domain name","copyof"+nudgepad.domain);$.get("/nudgepad.export",{},function(b){$("<form>",{action:"http://localhost/create",method:"post"}).append($("<input>",{name:"domain",value:a,type:"hidden"})).append($("<input>",{name:"email",value:nudgepad.cookie.email,type:"hidden"})).append($("<input>",{name:"timestamp",value:(new Date).getTime(),type:"hidden"})).append($("<input>",{name:"clone",value:b,type:"hidden"})).submit()})};
nudgepad.apps.develop.createFile=function(){var a=prompt("Name your file");if(!a)return!1;nudgepad.explorer.create(nudgepad.apps.develop.pathPretty+a,nudgepad.apps.develop.refresh)};nudgepad.apps.develop.home=function(){nudgepad.apps.develop.path="";nudgepad.apps.develop.pathPretty="";nudgepad.apps.develop.renderExplorer()};nudgepad.apps.develop.import=function(){nudgepad.textPrompt("Import a Site ","",function(a){$.post("/nudgepad.import",{space:a},function(){nudgepad.notify("Imported. Please restart")})})};
nudgepad.apps.develop.onopen=function(){nudgepad.apps.develop.log||nudgepad.apps.develop.refresh()};
...
(c.left=a),b>c.right&&(c.right=b),t<c.top&&(c.top=t),r>c.bottom&&(c.bottom=r)):(c.left=$(this).position().left,c.right=c.left+$(this).outerWidth(),c.top=$(this).position().top,c.bottom=c.top+$(this).outerHeight(),d=!0)});c.height=c.bottom-c.top;c.width=c.right-c.left;$(".deleteMe").remove();return c};nudgepad.apps.git=new App("git");nudgepad.apps.git.commit=function(){var a=prompt("Enter your commit message");nudgepad.apps.git.send("git commit -am '"+a+"'")};nudgepad.apps.git.init=function(){nudgepad.apps.git.send("git init")};
...
Grid.prototype.getDelta=function(a){this.eraseSnaplines();var b={},c=[],d=[],f;for(f in a){var n=a[f],t;for(t in this.points)if(this.points.hasOwnProperty(t)){var r=this.points[t],m=r.x-n.x,D=r.y-n.y;b.x||(b.x=m,b.y=D);Math.abs(m)<=Math.abs(b.x)&&(b.x=m,c=[n,r]);Math.abs(D)<=Math.abs(b.y)&&(b.y=D,d=[n,r])}}a=Math.abs(b.x)<this.radius;f=Math.abs(b.y)<this.radius;a||(b.x=0);f||(b.y=0);a&&(c[0]={x:c[0].x+b.x,y:c[0].y+b.y},this.drawSnapline(c[0],c[1]));f&&(d[0]={x:d[0].x+b.x,y:d[0].y+b.y},this.drawSnapline(d[0],
...
nudgepad.stage.insert=function(a,b,c,d,f){a||(a="scrap\n content Hello world\n style\n  position absolute\n  top 10px\n  left 10px");a=new Space(a.toString());nudgepad.stage.selection.clear();nudgepad.stage.insertBody();var n=nudgepad.pages.stage.get("body scraps");a.each(function(a){n.get(a)&&this.rename(a,n.autokey(a))});n.patch(a);var t=[];a.each(function(a,b){n.values[a]=new Scrap("body "+a,b);var c=n.values[a].render().element();c.length&&(c.selectMe(),t.push(n.values[a].selector()))});f&&(f=
...
nudgepad.StretchHandle.create=function(a,b,c,d){var f=a.element(),n="fixed"==f.css("position")?"fixed":"absolute",t=("top"==b?"n":"bottom"==b?"s":"")+("left"==c?"w":"right"==c?"e":""),r=$("<div></div>");r.attr("value",a.getPath());r.addClass("handle stretch_handle stretch_handle_"+a.id+" "+a.id+"_handle");r.attr("id","stretch_handle_"+b+c+a.id);r.css({cursor:t+"-resize",position:n,width:"5px",height:"5px",border:"1px solid rgba(0, 0, 0, .5)","background-color":"rgba(255, 255, 255, 0.5)","z-index":"50"});
...
$(this).parent().offset().top,r=nudgepad.StretchHandle.getLength(f,n,t,r,"center"!=c,"middle"!=b),t=a.scrap();"center"!=c&&(t.set("style left",(0<=r.x?f:r.x+f)+"px"),c=a.outerWidth()-a.width(),t.set("style width",Math.abs(r.x)-c+"px"));d?(b=parseFloat(t.get("style width"))/nudgepad.StretchHandle.originalWidth,t.set("style height",Math.round(nudgepad.StretchHandle.originalHeight*b)+"px")):"middle"!=b&&(0<=r.y?t.set("style top",n+"px"):t.set("style top",r.y+n+"px"),b=a.outerHeight(!0)-a.height(),t.set("style height",
...
n.append(c);c=$('<div class="editorButton"><img src="/nudgepad/images/lock_lt.png"></div>');c.on("tap",function(){nudgepad.stage.selection.patch("locked true");V.trigger("click");return!1});n.append(c);var r=$('<select id="nudgepadLinkSelect"></select>'),c=$("<option>No link</option>");c.on("click",function(){nudgepad.stage.selection.patch("href ")});r.append(c);_.each(site.values.pages.values,function(a,b){var c=$('<option value="'+b+'">'+ToProperCase(b)+"</option>");c.on("click",function(){nudgepad.stage.selection.patch("tag a\nhref "+
...
a&&nudgepad.stage.selection.patch("tag a\nhref "+a)});r.append(c);r.css({display:"none",position:"absolute","z-index":"100000"});r.attr("size",r.find("option").length);r.on("click",function(){r.hide();return!1});n.append(r);c=$('<div class="editorButton"><img src="/nudgepad/images/link_lt.png"></div>');c.on("tap",function(){r.css({left:$(this).position().left,top:$(this).position().top});r.show();return!1});n.append(c);c=$('<div class="editorButton"><img src="/nudgepad/images/up_lt.png" title="Increase z-index"></div>');
c.on("tap",function(){$(".selection").each(function(){$(this).scrap().moveUp()});G.val(a.get("style"));return!1});n.append(c);c=$('<div class="editorButton"><img src="/nudgepad/images/down_lt.png" title="Decrease z-index"></div>');c.on("tap",function(){$(".selection").each(function(){$(this).scrap().moveDown()});G.val(a.get("style"));return!1});n.append(c);var m=$('<div class="clear"></div>');n.append(m);c=$('<div class="columnLeft"></div>');n.append(c);f=$('<div class="editorHeader">Curve</div>');
...
nudgepad.stage.commit();$(".handle").trigger("update");return!1});c.append(f);f=$('<div class="toolButton toolButtonThree">5</div>');f.on("tap",function(){nudgepad.stage.selection.cssPreview("border 5px solid "+D);nudgepad.stage.commit();$(".handle").trigger("update");return!1});c.append(f);c=$('<div class="columnRight"></div>');n.append(c);f=$('<div class="editorHeader">Opacity</div>');c.append(f);f=$('<div class="toolButton toolButtonOne">1</div>');f.on("tap",function(){nudgepad.stage.selection.cssPreview("opacity 1");
...
nudgepad.pages.updateTabs=function(){$("#nudgepadTabs").html("");var a=site.get("pages").keys;_.each(a,function(a){var c=$("<span>"+a+"</span>");a===nudgepad.stage.activePage&&c.css("color","white");var d="";site.values.collage.each(function(f,n){if(n.get("page")!==a)return!0;d+=n.get("name")+"("+n.get("device")+") ";f!=nudgepad.id&&c.addClass("openPage")});c.attr("title",d);c.on("click",function(){mixpanel.track("I clicked a page tab");nudgepad.stage.open($(this).text());return!0});c.attr("value",
a);$("#nudgepadTabs").append(c)})};nudgepad.on("collage.update",nudgepad.pages.updateTabs);
nudgepad.textPrompt=function(a,b,c,d,f){var n=$('<textarea id="nudgepadEditorTextarea" class="nudgepad"></textarea>');n.val(b);var t=$('<div id="nudgepadEditorModalScreen" class="nudgepad"/>');t.on("tap mousedown click slide slidestart slideend mouseup",function(a){a.stopPropagation()});n.on("tap mousedown click slide slidestart slideend mouseup",function(a){a.stopPropagation()});if(d)n.on("keypress",d);var r=$('<div id="nudgepadEditorSaveButton" class="nudgepad">'+(f||"Save")+"</div>"),m=$('<div id="nudgepadEditorCancelButton" class="nudgepad">Cancel</div>'),
D=$('<div id="nudgepadEditorButtonContainer" class="nudgepad"></div>');t.on("click",function(){m.trigger("click")});m.on("click",function(){r.remove();n.remove();t.remove();D.remove();m.remove()});r.on("click",function(){c&&c(n.val());r.remove();n.remove();t.remove();D.remove();m.remove()});$("body").append(t);$("body").append(n);$("body").append(r);$("body").append(m);$("body").append(D);n.focus()};
/**
