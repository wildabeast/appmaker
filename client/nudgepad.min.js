...
"content",p=e.values.type;if(p&&p.match(/^(list)$/))return nudgepad.stage.selection.clear(),c.selectMe(),nudgepad.stage.selection.editSource(),!1;p&&p.match(/^(textarea|input|password)$/)&&(f="placeholder");nudgepad.textPrompt("Editing content for this block",e.values[f],function(a){e.values[f]=a;nudgepad.stage.commit();c.remove();$("#nudgepadStageBody").append(e.toHtml(null,!0));c.selectMe()})}};nudgepad.contentEditor.killEvent=function(){nudgepad.mouse.down.stopPropagation();return!1};
...
p.append(c);c=$('<div class="editorButton"><img src="/nudgepad/images/lock_lt.png"></div>');c.on("tap",function(){nudgepad.stage.selection.patch("locked true");N.trigger("click");return!1});p.append(c);var l=$('<select id="nudgepadLinkSelect"></select>'),c=$("<option>No link</option>");c.on("click",function(){nudgepad.stage.selection.patch("href ")});l.append(c);_.each(site.values.pages.values,function(a,b){var c=$('<option value="'+b+'">'+ToProperCase(b)+"</option>");c.on("click",function(){nudgepad.stage.selection.patch("type a\nhref "+
$(this).attr("value"))});l.append(c)});c=$("<option>New Page</option>");c.on("click",function(){var a=prompt("Enter the name of your new page",nudgepad.pages.nextName(nudgepad.stage.activePage));if(a){a=Permalink(a);nudgepad.stage.selection.patch("type a\nhref "+a);N.trigger("click");var b=nudgepad.stage.activePage;nudgepad.pages.duplicate(null,a,!0);nudgepad.stage.open(b)}});l.append(c);c=$("<option>External Link</option>");c.on("click",function(){var a=prompt("Enter the url to link to","http://");
