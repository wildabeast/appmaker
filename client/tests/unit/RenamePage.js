...
test("rename_page", function() {
 ...
 ok(site.get('pages ' + current_name) instanceof Space)
 equal(nudgepad.pages.rename(new_name), '')
 ok(!site.get('pages ' + current_name), 'Old page should be deleted')
 equal(new_name, nudgepad.stage.activePage, 'Current page should have new name')
 ok(site.get('pages ' + new_name) instanceof Space, 'New page should be instance of space')
