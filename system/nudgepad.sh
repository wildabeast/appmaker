...
'log')
  if [ -n "$2" ]
    then
      sudo cat $projectsPath/$2/nudgepad/logs/mon.txt
    else
      # Proxy log
...
'tail')
  if [ -n "$2" ]
    then
      sudo tail -n 30 -f $projectsPath/$2/nudgepad/logs/mon.txt
    else
      # Proxy log
...
'traffic')
  if [ -n "$2" ]
    then
      sudo tail -n 30 -f $projectsPath/$2/nudgepad/logs/requests.txt
    else
      # Proxy log
